...
if uploaded_file is not None:
    ...
    # Безопасное имя файла: берём только расширение, имя заменяем на uuid
    ...
    if st.button("Обработать запись", type="primary", use_container_width=True): # Большая кнопка которая начинает действие
        ...
        try:
            ...
            st.subheader("4. Протокол встречи")
            ...
            st.subheader("5. Задачи")
            st.write(zadachi)

            st.success("Готово")

        except Exception as e:
            # Ошибки (LM Studio не запущен, Whisper упал и т.д)
